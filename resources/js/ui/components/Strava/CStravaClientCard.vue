    ...
    >
        ...
        <v-card-text>
            <p style="text-align: center;">
                <v-btn @click="$inertia.post(route('strava.client.logout', client.id))" v-if="client.is_connected">
                    Logout
                </v-btn>
        ...
        <v-card-actions v-if="type === 'owned'">
            ...
            <div>
                <c-confirmation-dialog key="disableClient" v-if="client.enabled" title="Disable client?"
                                       button-text="Disable" :loading="isDisabling" cancel-button-text="Nevermind"
                                       @confirm="disableClient" ref="disableClientDialog">
                    <template v-slot:activator="{trigger,showing}">
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn
                ...
                <c-confirmation-dialog key="enableClient" v-else title="Enable client?" button-text="Enable"
                                       :loading="isEnabling" cancel-button-text="Nevermind" @confirm="enableClient"
                                       ref="enableClientDialog">
                    <template v-slot:activator="{trigger,showing}">
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn
            ...
            <div>
                <c-confirmation-dialog key="makeClientPrivate" v-if="client.public" title="Make client private?"
                                       button-text="Make private" :loading="isMakingPrivate"
                                       cancel-button-text="Nevermind" @confirm="makeClientPrivate"
                                       ref="makeClientPrivateDialog">
                    <template v-slot:activator="{trigger,showing}">
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn
                    ...
                    <p>Only you will have access to it. If you make it public again, anyone with access to it currently
                        will have access again.</p>
                </c-confirmation-dialog>

                <c-confirmation-dialog key="makeClientPublic" v-else title="Make client public?"
                                       button-text="Make public" :loading="isMakingPublic"
                                       cancel-button-text="Nevermind" @confirm="makeClientPublic"
                                       ref="makeClientPublicDialog">
                    <template v-slot:activator="{trigger,showing}">
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn
                    ...
                    <p>It will be instantly accessible for anyone else on the platform. We will not share your
                        information with them or vice versa.</p>
                </c-confirmation-dialog>
            ...
            <c-confirmation-dialog key="deleteClient" title="Delete client?" button-text="Delete" :loading="isDeleting"
                                   cancel-button-text="Nevermind" @confirm="deleteClient" ref="deleteClientDialog">
                <template v-slot:activator="{trigger,showing}">
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn
                ...
                <p>Anyone using the client will no longer be able to use it. If you later decide to use this client
                    again, you can add it as a new client.</p>
            </c-confirmation-dialog>
        ...
        <v-card-actions v-if="type === 'shared'">
            ...
            <c-confirmation-dialog key="sharedClient" title="Leave client?" button-text="Leave" :loading="isLeaving"
                                   cancel-button-text="Nevermind" @confirm="leaveClient" ref="leavingClientDialog">
                <template v-slot:activator="{trigger,showing}">
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn
                ...
                <p>You will no longer be able to use it, and will need a new link if you want to use it in the
                    future.</p>
            </c-confirmation-dialog>
...
export default {
    ...
    computed: {
        ...
        stravaLoginUrl() {
            return route('strava.client.login.start', {
