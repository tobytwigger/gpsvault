...
class ActivityIndexTest extends TestCase
{
    ...
    public function index_loads_activities_ordered_by_date(){
        ...
        $activities[0]->started_at = Carbon::now()->subDays(4);
        $activities[1]->started_at = Carbon::now()->subDays(2);
        $activities[2]->started_at = Carbon::now()->subDays(1);
        $activities[3]->started_at = Carbon::now()->subDays(11);
        $activities[4]->started_at = Carbon::now()->subDays(4);
        $activities->map->save();
    ...
    public function index_paginates_activities(){
        $this->authenticated();
        $activities = Activity::factory()->count(20)->create(['user_id' => $this->user->id, 'started_at' => null]);
    ...
    public function index_only_returns_your_activities(){
        $this->authenticated();
        $activities = Activity::factory()->count(3)->create(['user_id' => $this->user->id, 'started_at' => null]);
